...
| E | 地球から見たようにします |
| 0 | デフォルトの傾きに戻します |
| L S | 惑星位置をファイルに読込・保存します |
| SP | 現在の中心位置を固定します |

| マウス | 動作 |
| ---- | ---- |
| ホイール | 画面を拡大・縮小します |
| 左ドラッグ | XY軸を±90度まで傾けます |

#### 画面左上のデバッグ情報

| 名前 | 範囲 | 説明 |
| ---- | ---- | --- |
| Zoom | 0.010x - 10000x | 画面の拡大率です |
| Step | 0.001h - 1000h | 軌道計算の最小時間で精度と速度を決めます |
| / | 10 - ???? | ステップタイムの除算値でCPUやGPUの余力を表しています |
| Fps | 0.1 - 60.0 | 画面更新の時間で１秒あたりの更新回数です |
| Yser | 0 - ???? | シミュレーションの経過年数です |
| Days | 0 - 365 | シミュレーションの経過日数です |
| Time | 0 - 31556952 | シミュレーションの経過秒数です |

Step横の除算値とFpsでベンチマーク的な評価が出来ます。
除算値が大きい程CPU/GPUの計算速度が速く計算精度が高いです。
また、CPU/GPUが遅い場合は、Fps値が下がります。
初期値は、ソースコード内で決めていますので
ご自身の環境にあった精度と表示速度を探す必要があります。
ちなみに除算値に余裕があるとキー操作のTでステップタイムを上げても
小惑星や衛星などの軌道計算の破綻で飛び出しません。
...
衛星などのテクスチャ画像データは、下記より取得し縮小して使用しました。<BR>
